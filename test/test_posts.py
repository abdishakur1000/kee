...
def test_get_all_posts(authorized_client, test_posts):
    ...
    # TODO ARINTAAN KA SHAQAYSII IN AAD HESHO POST ID IYO CONTEN TIGA
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post('/posts/', json={
        'title': 'arbitrary title', 'content': "ksdsjflkjf"})

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == 'arbitrary title'
    assert created_post.content == "ksdsjflkjf"
...
def test_delete_posts_success(authorized_client, test_user, test_posts):
...
# TODO WAA IN AAD FAHAN TAA SABATA UU U SHAQEEN LA YAHAY
