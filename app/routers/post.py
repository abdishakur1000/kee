...
def get_posts(db: Session = Depends(get_db),
    ...
    # posts_current_user = db.query(models.Post).filter(models.Post.owner_id == current_user.id).all()
    """View all posts"""
    # TODO """WAX KA QAQO IF STATEMAN TIGAAN"""
    # if posts_current_user not in :
    #     print(posts_current_user)
    # else:
    #     print(posts)
    print(search)
...
def get_post(id: int, db: Session = Depends(get_db), current_user: int =
    ...
    '''YOU CAN ONLY VIEW YOUR OWN POST'''
    # if post.owner_id != current_user.id:
    #     raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
    #                         detail="This is not your post!!!AN-AUTHORIZED ACTION")
    return post
...
def deleting_post(id: int, db: Session = Depends(get_db),
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="This is not your post!!!AN-AUTHORIZED ACTION")
    if post.owner_id == post.owner_id:
        print(f'The user id {post.owner_id} with the Email {current_user.email} has Deleted post {id}')
...
def update_post(id: int, updated_post: schemas.PostCreate, db: Session = Depends(get_db),
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="This is not your post!!!AN-AUTHORIZED ACTION")
    if post.owner_id == post.owner_id:
        print(f'The user id {post.owner_id} with the Email {current_user.email} has updated post {id}')
